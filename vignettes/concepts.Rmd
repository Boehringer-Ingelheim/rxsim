...
cond_interim <- build_trigger(
  enroll_trigger(1.0, 4),
...
Compose helper triggers with `&` (AND) and `|` (OR), then either pass the
result directly as the `trigger` field in `analysis_generators` or
convert it to a `Condition` with `build_trigger()`.
...
cond_interim <- build_trigger(
  value_trigger("time", ">=", 12) & count_trigger("enroll_time", ">=", 15),
  analysis = my_analysis,
