...
Construct a `Condition` with `rlang::quos()` to capture the filter
predicates:
...
`Condition$new()` uses `rlang::quos()` instead of `rlang::exprs()`.
...
inside the expression when it is evaluated. Use `rlang::quos()` in
`Condition$new(where = ...)` and `rlang::exprs()` when supplying
triggers to `replicate_trial()`'s `analysis_generators`.
