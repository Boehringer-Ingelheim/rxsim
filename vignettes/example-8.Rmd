...
`d20` (20 mg), and `d40` (40 mg). The Emax truth has `emax = 0.9` and `ed50 =
...
convention as [Example 7](example-7.html)), while `alpha_final = 0.05` controls
...
The `enroll_plan` data frame specifies integer subject counts (`enroller`) per
...
throughout. `dropper` is set to 0 (no dropout). `tr_timer$get_end_timepoint()`
...
from generator functions), computing each arm's `n` from `sum(enroller)` in the
...
The same historical borrowing approach as [Example 7](example-7.html) is used
...
operator in `go_interim <<- as.integer(...)` writes the interim Go/No-Go
decision to the enclosing environment, making it available to downstream
analyses or conditional logic in the final step. The final analysis calls
