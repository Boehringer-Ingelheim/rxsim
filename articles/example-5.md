# Example 5: Seamless Ph2a/2b \| Two-arm -\> Multi-arm \| Single continuous endpoint \| Bayesian Go/No-Go + Bayesian MCP-Mod
...
Core setup follows the same pattern as [Example
1](https://boehringer-ingelheim.github.io/rxsim/articles/example-1.md).
The difference is multiple looks and stage-dependent enrollment logic.
...
The `enroll_plan` data frame specifies integer subject counts (`enroll`)
per arm per time unit. During time 1–4 (Ph2a), only `d0` and `d20`
enroll (8 per time each). From time 5 onward (Ph2b), three new doses
(`d5`, `d10`, `d40`) begin enrolling while the d20 arm tapers off; `d0`
continues at 8 per time throughout. `drop` is set to 0 (no dropout).
`tr_timer$get_end_timepoint()` confirms the last time point at which any
enrollment occurs.
...
`sum(enroll)` in the enrollment plan. This approach pre-generates the
...
For interim Go/No-Go, we use placebo borrowing from small historical
...
1.  `cond_interim` - fires when `time == interim_time` (calendar time
    4). Runs a Bayesian Go/No-Go analysis on d20 vs placebo. The `<<-`
    operator in `go_interim <<- as.integer(...)` writes the Go/No-Go
    decision to the enclosing environment, making it available to
    downstream logic in the final condition.
2.  `cond_final` - fires when `time == final_time` (calendar time 12).
    Calls
    [`BayesianMCPMod::assessDesign()`](https://boehringer-ingelheim.github.io/BayesianMCPMod/reference/assessDesign.html)
    with arm-level sample sizes (`n_patients`), the candidate
    dose-response models (`mods`), the five-arm prior list
    (`prior_list`), and arm-level posterior mean estimates
    (`estimates_sim`); `alpha_crit_val = alpha_final` sets the
    significance threshold for the Bayesian MCP contrast step.

Both conditions are passed to `Trial$new(conditions = ...)` and are
evaluated by `Trial$run()` at each timepoint.
...
#> $time_4
#> $time_4$interim_t4
#> $time_4$interim_t4$n_placebo
#> [1] 32
#> 
#> $time_4$interim_t4$n_d20
#> [1] 32
#> 
#> $time_4$interim_t4$post_prob_delta
#> [1] 0.9448993
#> 
#> $time_4$interim_t4$delta_q025
#> [1] 0.01981831
#> 
#> $time_4$interim_t4$delta_q500
#> [1] 0.4387862
#> 
#> $time_4$interim_t4$delta_q975
#> [1] 0.8449165
#> 
#> $time_4$interim_t4$decision_go
#> [1] 1
#> 
#> 
#> 
...
This example demonstrates the seamless setup and analysis flow in one
simulation run: 2-arm Ph2a enrollment, interim Bayesian Go/No-Go check,
then final Bayesian MCP-Mod after multi-arm expansion.

## Next steps

- [Conditions and
  Triggers](https://boehringer-ingelheim.github.io/rxsim/articles/conditions.md) -
  composing multiple looks with separate `Condition` objects
- [Two API
  Styles](https://boehringer-ingelheim.github.io/rxsim/articles/api-styles.md) -
  when to use direct object construction vs generator workflows
